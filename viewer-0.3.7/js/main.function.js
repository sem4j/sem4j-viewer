var server_url = 'http://sem4j.org:9001';
...
function addNeighbours(id, rel_type, type){
	...
	v_data = {
		"query" : 
            ...
            "WHERE n.type='" + type + "' " +
			"RETURN id(x), id(n), n.name, n.type, type(r) " +
			"LIMIT 1000",
	    "params" : {}
	};
	...
	// POST
	$.ajax({
		type: 'POST', url: server_url + '/cypher', data: v_data, dataType: 'json',
		success: function(result){
		...
		for(var i=0; i<rows.length; i++){
			...
			for (var j=0; j<rows[i].length; j++) {
				if (!cols[j].type) {
					...
					index = getNodeIndex(id);
					if (index == 0) {
						var name = n.data.name;
						var type = n.data.type;
						var node = { "id":id, "name":name, "type":type };
						//printLog(id);
						nodes.push(node);
					}
				}
			...
			for (var j=0; j<rows[i].length; j++) {
				if (cols[j].type) {
					var r = cols[j];
					var type = r.type;
					var index_source = getNodeIndex(uri2id(r.start));
					var index_target = getNodeIndex(uri2id(r.end));
					if (index_source != 0 && index_target != 0) {
						var link = { "source":nodes[index_source], "target":nodes[index_target], "type":type };
						//printLog(uri2id(r.start) + ' ' + uri2id(r.end) + ' ' + index_source + ' ' + index_target);
						links.push(link);
					}
				}
			}
